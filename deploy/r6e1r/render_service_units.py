#!/usr/bin/env python3
"""Render the sealed R6E1R user-service templates for one host layout."""
...
SCHEMA = "R6E1R_RENDERED_USER_UNITS_V1"
TEMPLATE_NAMES = (
    "r6e1r-shadow.service",
    "r6e1r-readonly-gateway.service",
)
TOKEN_PATTERN = re.compile(r"@[A-Z0-9_]+@")
SAFE_ABSOLUTE_PATH = re.compile(r"/(?:[A-Za-z0-9._-]+/)*[A-Za-z0-9._-]+\Z")
...
REQUIRED_TOKENS = {
    "r6e1r-shadow.service": frozenset({
        "@R6E1R_REPOSITORY_ROOT@",
        "@R6E1R_COLLECTOR_ROOT@",
        "@R6E1R_DEPLOY_ROOT@",
        "@R6E1R_PYTHON@",
    }),
    "r6e1r-readonly-gateway.service": frozenset({
        "@R6E1R_REPOSITORY_ROOT@",
        "@R6E1R_COLLECTOR_ROOT@",
        "@R6E1R_DEPLOY_ROOT@",
        "@R6E1R_GATEWAY_PORT@",
...
def _replacements(args: argparse.Namespace) -> Mapping[str, str]:
    ...
    if port < 8805 or port > 8810:
        raise RenderError("gateway port must be in the approved 8805-8810 range")
    ...
    protected = tuple(Path(value) for value in (
        repository, collector, deployment,
    ))
    for path in (*protected, Path(python)):
        if any(path == prefix or prefix in path.parents for prefix in SANDBOX_HIDDEN_PREFIXES):
    ...
    for index, left in enumerate(protected):
        for right in protected[index + 1:]:
            if left == right or left in right.parents or right in left.parents:
                raise RenderError("repository, collector, and deployment roots must not overlap")
    if Path(deployment) in Path(python).parents:
        raise RenderError("Python executable must not be below the writable deployment root")
    return {
...
def render_units(
    ...
    if template_root.is_symlink() or output_root.is_symlink():
        raise RenderError("unit template and output directories must not be symlinks")
    template_root = template_root.resolve()
    output_root = output_root.resolve()
    if output_root == template_root:
        raise RenderError("output directory must not replace the sealed templates")
    ...
    for name in TEMPLATE_NAMES:
        ...
        if not template.is_file() or template.is_symlink():
            raise RenderError("sealed unit template is unavailable")
        text = template.read_text(encoding="utf-8")
        observed_tokens = frozenset(TOKEN_PATTERN.findall(text))
        if observed_tokens != REQUIRED_TOKENS[name]:
            raise RenderError("sealed unit template token set is invalid")
        for token in observed_tokens:
            value = replacements[token]
            text = text.replace(token, value)
        if TOKEN_PATTERN.search(text):
            raise RenderError("unresolved sealed unit template token")
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        result = {"schema": SCHEMA, "ok": False, "error": "UNIT_RENDER_REFUSED"}
