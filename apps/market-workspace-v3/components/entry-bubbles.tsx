...
export function entryBubbleMarks(points:Row[],min:number,max:number):any {
  ...
  return {id:'near-otm-oi-entry-bubbles',name:'OI / VIX conditions',type:'custom',xAxisIndex:1,yAxisIndex:1,
    ...
    renderItem:(params:any,api:any)=>{
      ...
      const color=ENTRY_BUBBLE_COLORS[row.state as 'green'|'red'];
...
export const EntryBubbleReview=memo(function EntryBubbleReview({frame,min,show,onShow,onSeek,canSeek}:{
  ...
  return <div className="entry-bubble-review">
    <div className="entry-bubble-caption">
      ...
      {['PE','CE'].map(side=><span key={side}><b>{side} {side==='PE'?'above':'below'}</b> · <i className="entry-dot red"/> ↑ {events.filter(r=>r.side===side&&r.state==='red').length} · <i className="entry-dot green"/> ↓ {events.filter(r=>r.side===side&&r.state==='green').length}</span>)}
    ...
    <details className="entry-audit"><summary>Review {events.length} matches / {assessments.length} OI-spike reports in view</summary>
      <p>Three nearest strict OTM strikes at each report. OI fall is compared with the previous report and the median absolute percentage change of the previous 20 updates. Red: VIX rises ≥0.4%; green: VIX falls ≥0.4%.</p>
      ...
      <div className="entry-audit-list">{[...assessments].reverse().map(row=><div key={row.id} className="entry-audit-row">
        ...
        <div><strong className={row.status==='MATCH'?(row.state==='green'?'entry-green':'entry-red'):''}>{row.status==='MATCH'?category(row):`${row.side} OI spike · ${row.reason}`}</strong>
