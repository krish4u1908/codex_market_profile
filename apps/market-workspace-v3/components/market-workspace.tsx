"use client";
import {OiDirectionWorkspace} from './oi-direction-workspace';
...
import {BasisPanel,PriceBasisRibbonCaption,withPriceBasisRibbon} from './basis-panel';
import {EntryBubbleReview} from './entry-bubbles';
import {OiPressureStrip,OiPressureResearch,useStudy} from './oi-pressure-research';
import {OiPrimaryIndicator} from './oi-primary-indicator';
import '../app/oi-pressure.css';
import {researchStep} from '../public/oi-research-replay.mjs';
import {OptionOiFlowRibbon} from './option-oi-flow-ribbon';
...
const MarketCharts=memo(function MarketCharts({frame,flags,scope,windowMinutes,onLevelsChange,onSeek,canSeek,oiResearchEligible,priceOnly=false}:{frame:Frame;flags:Flags;scope:string;windowMinutes:number;onLevelsChange:(show:boolean)=>void;onSeek:(value:number)=>void;canSeek:boolean;oiResearchEligible:boolean;priceOnly?:boolean}){
  ...
  const chartOptions=useMemo(()=>{
    ...
    const vix={...baseChart(min,max),series:[line('India VIX · value',frame.cash,'vix_close',COLORS.vix,{lineStyle:{color:COLORS.vix,width:1.8,type:'dashed'}})],
      tooltip:{...(baseChart(min,max).tooltip as object),formatter:(raw:any)=>{
        const p=Array.isArray(raw)?raw[0]:raw,row=frame.cash.find(r=>r.x===p?.value?.[0]);
        return `<strong>India VIX · ${fmt(row?.vix_close)}</strong><br/>${frame.cashVixQuality.sourceTime?'Minute close':'Receipt'} ${clock(row?.x,true)} IST`
          +(row?.available_at?`<br/>Available ${clock(row.available_at,true)} IST`:'')
          +(row?.revision?`<br/>Data revision ${row.revision}`:'')
          +(row?.vix_close==null?'<br/>Missing reading':'');
      }}};
    ...
    return {price:withPriceBasisRibbon({...priceBase,series},frame,min,max,showEntries),vix,cash,oi};
  },[frame,flags,scope,min,max,visibleLevels,climaxVisible,optionPoints,showEntries]);
  return <div className="chart-stack">
    <ChartPanel title={frame.profile.label} subtitle="Index · 1m view" value={fmt(frame.latest?.i)} colour={COLORS.price} className="price-panel">
      ...
      <Plot label={`${frame.profile.label} index with OI-VPOC levels and a three-minute price/basis ribbon. Translucent vertical VIX marks: red for a five-minute rise of at least 0.4%, green for a fall of at least 0.4%. OI-fall bubbles: green PE with VIX rise above; red CE with VIX fall below; other combinations yellow for research.`} option={chartOptions.price} height={380}/>
      <PriceBasisRibbonCaption frame={frame}/>
      {frame.profile.instrument!=='NIFTY'&&<OptionOiFlowRibbon frame={frame} min={min} max={max}/>}
      <EntryBubbleReview frame={frame} min={min} show={showEntries} onShow={setShowEntries} onSeek={onSeek} canSeek={canSeek}/>
      <div className="chart-footer"><span><Focus size={13}/> Shared time cursor</span><span>Ctrl + scroll to zoom · pinch on touch</span><span>{clock(min)} – {clock(max)} IST</span></div>
      {frame.profile.version==='2.0.0'&&<OptionClimaxDetails points={optionPoints} allPoints={allClimaxPoints}/>}
    </ChartPanel>
    {!priceOnly&&<>
    {flags.flows&&<OiPrimaryIndicator frame={frame} eligible={oiResearchEligible} min={min} max={max} showVix={flags.vix} showBasis={flags.basis}/>}
    {frame.profile.version==='2.0.0'&&<V2VolumePanel frame={frame} min={min} max={max}/>}
    {!(frame.profile.instrument==='NIFTY'&&frame.profile.version==='2.0.0'&&flags.flows)&&flags.vix&&<ChartPanel title="India VIX" subtitle={frame.cashVixQuality.sourceTime?'1m close · dashed':'Actual value · dashed'} value={fmt(frame.cash.at(-1)?.vix_close)} colour={COLORS.vix}>
      <Plot option={chartOptions.vix} label="India VIX actual retained values, with missing minutes left blank" height={138}/>
      <p className="chart-source-note" aria-label="VIX data quality">{frame.cashVixQuality.valid}/{frame.cashVixQuality.total} VIX minutes available · {frame.cashVixQuality.missing} missing
        {frame.cashVixQuality.sourceTime?' · Source-minute time; hover for arrival time':' · Original receipt history'}
        {frame.cashVixQuality.status==='SOURCE_UNAVAILABLE'||frame.cashVixQuality.status==='ERROR'?' · Input reader unavailable':''}</p>
    </ChartPanel>}
    ...
    {!(frame.profile.instrument==='NIFTY'&&frame.profile.version==='2.0.0'&&flags.flows)&&flags.basis&&<BasisPanel frame={frame} min={min} max={max}/>}
    ...
    {!(frame.profile.instrument==='NIFTY'&&frame.profile.version==='2.0.0')&&flags.flows&&(frame.profile.version==='2.0.0'&&!frame.capabilities.strikeReceipts?<V2FlowCharts frame={frame} min={min} max={max}/>:<><OptionFlow frame={frame} side="CE" metric="d" min={min} max={max}/><OptionFlow frame={frame} side="PE" metric="d" min={min} max={max}/></>)}
    </>}
  </div>;
});
...
function EventWorkspace({frame,onSeek,canSeek}:{frame:Frame;onSeek:(n:number)=>void;canSeek:boolean}){
  ...
  const entries=(frame.entryBubbles||[]).map(r=>({...r,x:Number(r.x),eventType:'Entry',label:`${r.watch==='LONG_WATCH'?'Long watch':r.watch==='SHORT_WATCH'?'Short watch':'Research'} · ${r.side} OI fall · VIX ${r.vixDirection==='UP'?'↑':'↓'} · ${r.spikes.map((s:Row)=>fmt(s.strike,0)+' '+s.side).join(', ')}`}));
  const events=[...entries,...frame.transitions.map(r=>({...r,x:Number(r.x),eventType:'Divergence',label:`${r.colour} · ${words(r.state)}`})),...shifts].sort((a,b)=>b.x-a.x).filter(r=>filter==='all'||r.eventType===filter).slice(0,100);
  return <section className="table-card"><div className="section-bar"><div><h2>Published events</h2><p>Only events available at {clock(frame.now,true)} IST · latest 100</p></div><Choice label="Event type" value={filter} onChange={setFilter} choices={[{value:'all',label:'All events'},{value:'VPOC',label:'VPOC shifts'},{value:'Divergence',label:'Divergence'},{value:'Entry',label:'OI / VIX bubbles'}]}/></div><div className="event-list">{events.map((e,i)=><button key={i} className="event-row" disabled={!canSeek} title={canSeek?"Seek to event":"Switch to Replay to seek"} onClick={()=>onSeek(e.x)}><span className="event-time">{clock(e.x,true)}</span><span className={`event-type ${e.eventType==='VPOC'?'vpoc':''}`}>{e.eventType}</span><span>{e.label}</span><ArrowUpRight size={15}/></button>)}{!events.length&&<p className="empty-copy">No events published at this time.</p>}</div></section>;
}
...
export default function MarketWorkspace({profileId,onProfileChange,workspaceConfig}:{profileId:string;onProfileChange:(id:string)=>void;workspaceConfig:Row}){
  ...
  const [cursor,setCursor]=useState(0),[playing,setPlaying]=useState(false),[speed,setSpeed]=useState('15'),[step,setStep]=useState('5'),[tab,setTab]=useState('market');
  ...
  const [pauseOnOiChange,setPauseOnOiChange]=useState(true);
  const oiEligible=mode==='replay'&&!imported.current&&(session.startsWith(`rebuilt-${frame?.session}`)||session==='recorded-2026-09-15');
  const oiStudy=useStudy(frame,oiEligible);
  const oiReplay=useRef<{rows:any[]|undefined;pause:boolean}>({rows:undefined,pause:false});
  oiReplay.current={rows:oiStudy.series?.rows,pause:pauseOnOiChange&&oiEligible};
  useEffect(()=>{
  ...
  useEffect(()=>{if(!playing||!meta)return;const timer=setInterval(()=>setCursor(prev=>{const tick=researchStep(oiReplay.current.rows,prev,meta.end,oiReplay.current.pause);if(tick.paused||tick.now>=meta.end)setPlaying(false);return tick.now;}),Number(speed)*1000);return()=>clearInterval(timer);},[playing,meta,speed]);
  ...
  useEffect(()=>{if(tab!=='oi-research'||mode!=='replay')return;const handle=(e:KeyboardEvent)=>{const el=e.target as HTMLElement;if(e.altKey||e.ctrlKey||e.metaKey||el.closest('input,select,textarea,button,[role=slider],[role=combobox],[role=tab],[contenteditable=true]'))return;if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();seek(cursor+(e.key==='ArrowLeft'?-60000:60000));}};document.addEventListener('keydown',handle);return()=>document.removeEventListener('keydown',handle);},[tab,mode,cursor,seek]);
  ...
  return <div className="market-app">
    ...
    <main className="workspace-shell">
      ...
      <section className="ticker-strip" aria-label={mode==='live'?'Latest market values':'Market values at the replay cursor'}><div className="ticker-primary"><span className="ticker-label">{profile.label.toUpperCase()} INDEX</span><div><strong>{fmt(latest?.i)}</strong><span style={{color:colorFor(change)}}>{change!=null&&<>{change>=0?<ArrowUpRight size={17}/>:<ArrowDownRight size={17}/>} {signed(change)} <small>({signed(first?change/first*100:0)}%)</small></>}</span></div><span className="ticker-note">Change from first session receipt</span>{frame?.provenance.price_note&&<span className="ticker-note">{frame.provenance.price_note}</span>}<dl className="ticker-session-range"><div><dt>High so far</dt><dd>{fmt(frame?.range.high)}</dd></div><div><dt>Low so far</dt><dd>{fmt(frame?.range.low)}</dd></div></dl></div><div className="ticker-item"><span>Futures basis</span><strong style={{color:COLORS.basis}}>{signed(latest?.b)} <small>pts</small></strong><span>Futures {fmt(latest?.f)}</span><dl className="ticker-session-range ticker-compact-range" aria-label="Futures basis high and low so far in points"><div><dt>High</dt><dd>{signed(frame?.basisRange.high)}</dd></div><div><dt>Low</dt><dd>{signed(frame?.basisRange.low)}</dd></div></dl></div><div className="ticker-item"><span>India VIX</span><strong style={{color:COLORS.vix}}>{fmt(frame?.cash.at(-1)?.vix_close)}</strong><span>Receipt {clock(frame?.cash.at(-1)?.available_at||frame?.cash.at(-1)?.t||frame?.cash.at(-1)?.x)}</span><dl className="ticker-session-range ticker-compact-range" aria-label="Recorded VIX high and low at the replay time"><div><dt>High</dt><dd>{fmt(frame?.vixRange.high)}</dd></div><div><dt>Low</dt><dd>{fmt(frame?.vixRange.low)}</dd></div></dl><span className="vix-range-source">{frame?.vixRange.from?`1m closes from ${clock(frame.vixRange.from)}`:"Awaiting VIX readings"}</span></div><div className="ticker-item"><span>Futures OI</span><strong style={{color:COLORS.oi}}>{compact(frame?.oi.at(-1)?.oi)}</strong><span>Δ {signed(frame?.oi.at(-1)?.d,0)}</span></div><div className="ticker-item ticker-call"><span>Current call</span><strong style={{color:frame?.call?.direction==='UP'?COLORS.positive:frame?.call?.direction==='DOWN'?COLORS.negative:COLORS.muted}}>{frame?.call?.direction||'WAIT'} <small>{signed(frame?.call?.score)}</small></strong><span>{words(frame?.call?.confidence)} confidence</span></div></section>
      <Tabs value={tab} onValueChange={setTab} className="workspace-tabs"><div className="tab-toolbar"><TabsList variant="line" className="main-tab-list"><TabsTrigger value="market"><Activity size={15}/> Market</TabsTrigger><TabsTrigger value="options"><BarChart3 size={15}/> Options</TabsTrigger><TabsTrigger value="inventory"><Layers3 size={15}/> Inventory</TabsTrigger><TabsTrigger value="events"><Clock3 size={15}/> Events</TabsTrigger>{profile.instrument==='NIFTY'&&profile.version==='2.0.0'&&<><TabsTrigger value="oi-research"><Activity size={15}/> OI Research</TabsTrigger><TabsTrigger value="oi-direction"><BarChart3 size={15}/> OI Direction</TabsTrigger></>}</TabsList><div className="range-controls"><Choice value={scope} onChange={setScope} label="Chart control scope" choices={[{value:'ID',label:'Intraday VPOC'},{value:'1D',label:'1D prior VPOC'},{value:'2D',label:'2D prior VPOC'},{value:'3D',label:'3D prior VPOC'}]}/><div className="range-buttons" aria-label="Visible time range">{[{v:'30',l:'30m'},{v:'60',l:'1h'},{v:'0',l:'Session'}].map(r=><button key={r.v} aria-pressed={windowMinutes===r.v} className={windowMinutes===r.v?'active':''} onClick={()=>setWindowMinutes(r.v)}>{r.l}</button>)}</div></div></div>
        {loading&&!frame?<div className="workspace-loading" role="status"><Loader2 className="spin" size={25}/><h2>{mode==='live'?'Connecting to the shared core':'Loading recorded market data'}</h2><p>Preparing price, OI and saved calls.</p></div>:frame?<div className="workspace-grid"><div className="main-column"><OiPressureStrip frame={frame} eligible={mode==='replay'&&!imported.current&&(session.startsWith('rebuilt-')||session==='recorded-2026-09-15')} onOpen={()=>setTab('oi-research')}/><TabsContent value="market"><MarketCharts frame={frame} flags={flags} scope={scope} windowMinutes={Number(windowMinutes)} onLevelsChange={levels=>setFlags(previous=>({...previous,levels}))} onSeek={seek} canSeek={mode==='replay'} oiResearchEligible={mode==='replay'&&!imported.current&&(session.startsWith('rebuilt-')||session==='recorded-2026-09-15')}/></TabsContent><TabsContent value="oi-direction">{profile.instrument==='NIFTY'&&profile.version==='2.0.0'&&<OiDirectionWorkspace frame={frame} windowMinutes={Number(windowMinutes)} priceChart={<MarketCharts frame={frame} flags={flags} scope={scope} windowMinutes={Number(windowMinutes)} onLevelsChange={levels=>setFlags(previous=>({...previous,levels}))} onSeek={seek} canSeek={mode==='replay'} oiResearchEligible={false} priceOnly/>}/>}</TabsContent><TabsContent value="options"><OptionWorkspace frame={frame} windowMinutes={Number(windowMinutes)}/></TabsContent><TabsContent value="inventory"><InventoryWorkspace frame={frame} scope={scope} onScope={setScope}/></TabsContent><TabsContent value="events"><EventWorkspace frame={frame} onSeek={seek} canSeek={mode==='replay'}/></TabsContent><TabsContent value="oi-research">{profile.instrument==='NIFTY'&&profile.version==='2.0.0'&&<OiPressureResearch frame={frame} eligible={mode==='replay'&&!imported.current&&(session.startsWith('rebuilt-')||session==='recorded-2026-09-15')}/>}</TabsContent></div><aside className="insights-column">{profile.version==='2.0.0'&&<V2Context frame={frame}/>}<CallCard frame={frame} onSummary={()=>summary.current?.open()}/><SummaryPanel ref={summary} frame={frame}/><LevelList frame={frame} scope={scope}/><OptionOIProfile frame={frame}/></aside></div>:null}
      </Tabs>
