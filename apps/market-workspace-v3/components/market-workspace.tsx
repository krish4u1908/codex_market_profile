...
import {useDisplayPreview,PreviewStatus,PreviewPricePlot,PreviewVixPlot,IncomingVolume} from './display-preview';
import {liveReading,incomingIndex as readIncomingIndex} from '../public/display-preview.mjs';
...
const MarketCharts=memo(function MarketCharts({frame,flags,scope,windowMinutes,onLevelsChange,preview,previewEnabled}:{frame:Frame;flags:Flags;scope:string;windowMinutes:number;onLevelsChange:(show:boolean)=>void;preview:Row|null;previewEnabled:boolean}){
  ...
  const chartOptions=useMemo(()=>{
    ...
    return {price:withPriceBasisRibbon({...priceBase,series},frame,min,max),vix,cash,oi};
  },[frame,flags,scope,min,max,visibleLevels,climaxVisible,optionPoints]);
  return <div className="chart-stack">
    <ChartPanel title={frame.profile.label} subtitle={previewEnabled?"Confirmed history · dotted incoming price":"Index · 1m view"} value={fmt(readIncomingIndex(preview,frame)?.price??frame.latest?.i)} colour={COLORS.price} className="price-panel">
      ...
      <PreviewPricePlot frame={frame} data={preview} label={`${frame.profile.label} confirmed price and climax markers, with provisional incoming index readings`} option={chartOptions.price}/>
    ...
    {previewEnabled&&<IncomingVolume data={preview} frame={frame}/>}
    {frame.profile.version==='2.0.0'&&<V2VolumePanel frame={frame} min={min} max={max}/>}
    {flags.vix&&<ChartPanel title="India VIX" subtitle={frame.cashVixQuality.sourceTime?'1m close · dashed':'Actual value · dashed'} value={fmt(liveReading(preview,'NSE:INDIAVIX-INDEX')?.price??frame.cash.at(-1)?.vix_close)} colour={COLORS.vix}>
      <PreviewVixPlot frame={frame} data={preview} option={chartOptions.vix}/>
...
export default function MarketWorkspace({profileId,onProfileChange,workspaceConfig}:{profileId:string;onProfileChange:(id:string)=>void;workspaceConfig:Row}){
  ...
  const previewEnabled=workspaceConfig.realtimePreview===true&&mode==='live';
  const preview=useDisplayPreview(previewEnabled,profile.instrument,frame?.session);
  ...
  const latest=frame?.latest,first=frame?.range.open;
  const incomingIndex=readIncomingIndex(preview.data,frame),incomingVix=liveReading(preview.data,'NSE:INDIAVIX-INDEX');
  const displayIndex=incomingIndex?.price??latest?.i,change=displayIndex!=null&&first!=null?displayIndex-first:null;
  const incomingPrices=incomingIndex?preview.data!.prices.map((r:Row)=>r.price):[];
  const displayHigh=frame?.range.high==null?null:Math.max(frame.range.high,...incomingPrices),displayLow=frame?.range.low==null?null:Math.min(frame.range.low,...incomingPrices);
  return <div className="market-app">
    ...
    <main className="workspace-shell">
      ...
      {previewEnabled&&<PreviewStatus data={preview.data} error={preview.error}/>}
      ...
      <section className="ticker-strip" aria-label={mode==='live'?'Latest market values':'Market values at the replay cursor'}><div className="ticker-primary"><span className="ticker-label">{profile.label.toUpperCase()} INDEX</span><div><strong>{fmt(displayIndex)}</strong><span style={{color:colorFor(change)}}>{change!=null&&<>{change>=0?<ArrowUpRight size={17}/>:<ArrowDownRight size={17}/>} {signed(change)} <small>({signed(first?change/first*100:0)}%)</small></>}</span></div><span className="ticker-note">{incomingIndex?`Incoming · ${clock(incomingIndex.received_ms,true)} IST · provisional`:"Change from first session receipt"}</span>{frame?.provenance.price_note&&<span className="ticker-note">{frame.provenance.price_note}</span>}<dl className="ticker-session-range"><div><dt>High so far</dt><dd>{fmt(displayHigh)}</dd></div><div><dt>Low so far</dt><dd>{fmt(displayLow)}</dd></div></dl></div><div className="ticker-item"><span>Futures basis</span><strong style={{color:COLORS.basis}}>{signed(latest?.b)} <small>pts</small></strong><span>Futures {fmt(latest?.f)}</span><dl className="ticker-session-range ticker-compact-range" aria-label="Futures basis high and low so far in points"><div><dt>High</dt><dd>{signed(frame?.basisRange.high)}</dd></div><div><dt>Low</dt><dd>{signed(frame?.basisRange.low)}</dd></div></dl></div><div className="ticker-item"><span>India VIX</span><strong style={{color:COLORS.vix}}>{fmt(incomingVix?.price??frame?.cash.at(-1)?.vix_close)}</strong><span>Receipt {clock(incomingVix?.received_ms||frame?.cash.at(-1)?.available_at||frame?.cash.at(-1)?.t||frame?.cash.at(-1)?.x)}{incomingVix?' · provisional':''}</span><dl className="ticker-session-range ticker-compact-range" aria-label="Recorded VIX high and low at the replay time"><div><dt>High</dt><dd>{fmt(frame?.vixRange.high)}</dd></div><div><dt>Low</dt><dd>{fmt(frame?.vixRange.low)}</dd></div></dl><span className="vix-range-source">{frame?.vixRange.from?`1m closes from ${clock(frame.vixRange.from)}`:"Awaiting VIX readings"}</span></div><div className="ticker-item"><span>Futures OI</span><strong style={{color:COLORS.oi}}>{compact(frame?.oi.at(-1)?.oi)}</strong><span>Δ {signed(frame?.oi.at(-1)?.d,0)}</span></div><div className="ticker-item ticker-call"><span>Current call</span><strong style={{color:frame?.call?.direction==='UP'?COLORS.positive:frame?.call?.direction==='DOWN'?COLORS.negative:COLORS.muted}}>{frame?.call?.direction||'WAIT'} <small>{signed(frame?.call?.score)}</small></strong><span>{words(frame?.call?.confidence)} confidence</span></div></section>
      <Tabs value={tab} onValueChange={setTab} className="workspace-tabs"><div className="tab-toolbar"><TabsList variant="line" className="main-tab-list"><TabsTrigger value="market"><Activity size={15}/> Market</TabsTrigger><TabsTrigger value="options"><BarChart3 size={15}/> Options</TabsTrigger><TabsTrigger value="inventory"><Layers3 size={15}/> Inventory</TabsTrigger><TabsTrigger value="events"><Clock3 size={15}/> Events</TabsTrigger></TabsList><div className="range-controls"><Choice value={scope} onChange={setScope} label="Chart control scope" choices={[{value:'ID',label:'Intraday VPOC'},{value:'1D',label:'1D prior VPOC'},{value:'2D',label:'2D prior VPOC'},{value:'3D',label:'3D prior VPOC'}]}/><div className="range-buttons" aria-label="Visible time range">{[...(previewEnabled?[{v:'5',l:'5m'}]:[]),{v:'30',l:'30m'},{v:'60',l:'1h'},{v:'0',l:'Session'}].map(r=><button key={r.v} aria-pressed={windowMinutes===r.v} className={windowMinutes===r.v?'active':''} onClick={()=>setWindowMinutes(r.v)}>{r.l}</button>)}</div></div></div>
        {loading&&!frame?<div className="workspace-loading" role="status"><Loader2 className="spin" size={25}/><h2>{mode==='live'?'Connecting to the shared core':'Loading recorded market data'}</h2><p>Preparing price, OI and saved calls.</p></div>:frame?<div className="workspace-grid"><div className="main-column"><TabsContent value="market"><MarketCharts preview={preview.data} previewEnabled={previewEnabled} frame={frame} flags={flags} scope={scope} windowMinutes={Number(windowMinutes)} onLevelsChange={levels=>setFlags(previous=>({...previous,levels}))}/></TabsContent><TabsContent value="options"><OptionWorkspace frame={frame} windowMinutes={Number(windowMinutes)}/></TabsContent><TabsContent value="inventory"><InventoryWorkspace frame={frame} scope={scope} onScope={setScope}/></TabsContent><TabsContent value="events"><EventWorkspace frame={frame} onSeek={seek} canSeek={mode==='replay'}/></TabsContent></div><aside className="insights-column">{profile.version==='2.0.0'&&<V2Context frame={frame}/>}<CallCard frame={frame} onSummary={()=>summary.current?.open()}/><SummaryPanel ref={summary} frame={frame}/><LevelList frame={frame} scope={scope}/><OptionOIProfile frame={frame}/></aside></div>:null}
      </Tabs>
