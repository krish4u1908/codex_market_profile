...
const MarketCharts=memo(function MarketCharts({frame,flags,scope,windowMinutes,onLevelsChange,onSeek,canSeek}:{frame:Frame;flags:Flags;scope:string;windowMinutes:number;onLevelsChange:(show:boolean)=>void;onSeek:(value:number)=>void;canSeek:boolean}){
  ...
  return <div className="chart-stack">
    <ChartPanel title={frame.profile.label} subtitle="Index · 1m view" value={fmt(frame.latest?.i)} colour={COLORS.price} className="price-panel">
      ...
      <Plot label={`${frame.profile.label} index with OI-VPOC levels and a three-minute price/basis ribbon. Translucent vertical VIX marks: red for a five-minute rise of at least 0.4%, green for a fall of at least 0.4%. Translucent OI-fall bubbles: PE above the ribbon, CE below; red for VIX rise and green for VIX fall.`} option={chartOptions.price} height={380}/>
...
function EventWorkspace({frame,onSeek,canSeek}:{frame:Frame;onSeek:(n:number)=>void;canSeek:boolean}){
  ...
  const entries=(frame.entryBubbles||[]).map(r=>({...r,x:Number(r.x),eventType:'Entry',label:`${r.side} OI fall · VIX ${r.vixDirection==='UP'?'↑':'↓'} · ${r.spikes.map((s:Row)=>fmt(s.strike,0)+' '+s.side).join(', ')}`}));
