...
function EventWorkspace({frame,onSeek}:{frame:Frame;onSeek:(n:number)=>void}){
...
export default function MarketWorkspace({profileId,onProfileChange}:{profileId:string;onProfileChange:(id:string)=>void}){
  ...
  useEffect(()=>{
    ...
    fetch(profile.catalog,{signal:abort.signal}).then(r=>{if(r.status===404)return {sessions:[]};if(!r.ok)throw new Error('Session catalog is unavailable.');return r.json();}).then(c=>{setCatalog(c.sessions||[]);if(!imported.current){setSession(c.sessions?.[0]?.session||'');if(!c.sessions?.length)setLoading(false);}}).catch(e=>{if(e.name!=='AbortError'){setError(e.message);setLoading(false);}});
    ...
    return()=>{abort.abort();w.terminate();};
  },[]);
  useEffect(()=>{if(!session||!worker.current)return;const entry=catalog.find(r=>r.session===session);if(!entry)return;setPlaying(false);setLoading(true);setError('');setFrame(null);setMeta(null);worker.current.postMessage({id:++request.current,action:'load',url:entry.payload,profileId});},[session,catalog]);
  useEffect(()=>{if(!meta||!worker.current)return;worker.current.postMessage({id:++request.current,action:'frame',now:cursor});},[cursor,meta,session]);
  ...
  const seek=useCallback((value:number)=>{if(meta){setPlaying(false);setCursor(Math.max(meta.start,Math.min(meta.end,value)));}},[meta]);
  ...
  return <div className="market-app">
    <header className="app-header"><a className="brand" href="/" aria-label={`${profile.label} workspace`}><span className="brand-icon"><Activity size={21}/></span><span>{profile.instrument} <b>WORKSPACE</b></span><span className="version-badge">V3</span></a><div className="header-status"><span className="recorded-label"><span/> Recorded session</span><span className="header-baseline">v{profile.version} engine</span><button className="icon-button" aria-label="Display settings" onClick={()=>setSettings(true)}><Settings2 size={18}/></button></div></header>
    <main className="workspace-shell">
      <div className="workspace-toolbar"><div className="toolbar-left"><Choice label="Instrument and engine version" value={profileId} onChange={onProfileChange} choices={Object.values(PROFILES).map((p:any)=>({value:p.id,label:p.title}))}/><span className="mode-badge"><RotateCcw size={13}/> Replay</span><CalendarDays size={15} className="calendar-icon"/>{!imported.current&&catalog.length>0&&<Choice label="Recorded session" value={session} onChange={changeSession} choices={catalog.map(r=>({value:r.session,label:dateLabel(r.session)}))}/>}</div><div className="toolbar-right"><label className="secondary-button file-picker">Open session<input type="file" accept=".json,.gz" aria-label={`Open ${profile.title} session file`} onChange={e=>{openFile(e.target.files?.[0]);e.target.value='';}}/></label><button className="secondary-button" onClick={()=>summary.current?.open()} disabled={!frame}><MessageSquareText size={15}/> Market summary</button><button className="secondary-button settings-button" onClick={()=>setSettings(true)}><Settings2 size={15}/> Customize</button></div></div>
