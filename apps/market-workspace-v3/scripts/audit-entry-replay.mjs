...
const {values}=parseArgs({options:{input:{type:'string'},profile:{type:'string'},'indicator-inputs':{type:'string'},output:{type:'string'}}});
...
const report={policy:data.entryAnalysis.policy,profile:values.profile,session:data.session,
  ...
  counts:{long:data.entryAnalysis.events.filter(r=>r.direction==='LONG').length,
    short:data.entryAnalysis.events.filter(r=>r.direction==='SHORT').length,spikeReports:data.entryAnalysis.assessments.length},
...
console.log(JSON.stringify({profile:report.profile,session:report.session,counts:report.counts,
  ...
  events:report.events.map(row=>({ist:row.ist,direction:row.direction,strikes:row.spikes.map(s=>s.strike)}))},null,2));
