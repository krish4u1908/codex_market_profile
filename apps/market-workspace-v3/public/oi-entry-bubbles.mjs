...
export const OI_ENTRY_POLICY=Object.freeze({id:'BASIC_OTM_OI_VIX_V1',strikesPerSide:3,
  ...
  pePlacement:'ABOVE',cePlacement:'BELOW',vixRiseColor:'red',vixFallColor:'green'});
  ...
  for(const r of feed.reports) {
    ...
    for(const side of ['PE','CE'])if(grouped[side].length) {
      ...
      const row={id:`${OI_ENTRY_POLICY.id}:${data.session}:${r.x}:${side}`,x:r.x,side,spikes:grouped[side],
        status:eligible?'MATCH':'OI_ONLY',category:direction?`${side}_${direction}`:null,
        vixDirection:direction,vixWindow,state:direction==='UP'?'red':'green',placement:side==='PE'?'ABOVE':'BELOW',
        reason:validVix?'VIX five-minute change below 0.4%':'Incomplete five-minute report-quote window'};
