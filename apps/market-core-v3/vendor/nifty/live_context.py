...
class LiveContext:
    def __init__(self, state_directory, collector_root, poll_seconds=10):
    ...
    def payload(self, day=None):
        ...
        with self.lock:
            ...
            return dict(version='2.0.0', baseline_version='1.0.62',session=day,decisions=rows,
                        price_history=list(minutes.values()),live_status=self.status())
        ...
        while not self.stop.is_set():
            try:
                if self.source is None:
                    ...
                    self.source = DirectSource(self.collector_root,self.root/'engine')
