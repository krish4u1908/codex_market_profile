"""Canonical raw-only NIFTY-reference multi-horizon inventory engine."""
...
import numpy as np
import pandas as pd
from v2_engine.raw_io.reader import load_market,load_oi,select_contracts,backward_join,moneyness

FAMILIES=("NIFTY_REF_FUT_VOLUME_VPOC","FUT_POS_OI_VPOC","FUT_NEG_OI_VPOC","CE_POS_OI_VPOC","CE_NEG_OI_VPOC","PE_POS_OI_VPOC","PE_NEG_OI_VPOC")
 ...
 for date in evaluations:
  ...
  for horizon,count in (("1D",1),("2D",2),("3D",3)):
   ...
   for family in FAMILIES:
    q=pd.concat([frames[d]["price"] if family=="NIFTY_REF_FUT_VOLUME_VPOC" else frames[d]["oi"][frames[d]["oi"].family==family] for d in sources],ignore_index=True);result=profile(q,config["bin_points"])
    if result:
     latest=iso(q.receipt_timestamp.max())
     output.append(record(date,horizon,family,result["control_value"],"|".join(sources),date+"T09:15:00+05:30",latest,futures,fe if family.startswith(("NIFTY_","FUT_")) else oe,result["count"],result["winning_bin_weight"],result["runner_up_bin"],result["runner_up_weight"],result["tie_break_reason"]))
  for family in FAMILIES:
   q=frames[date]["price"] if family=="NIFTY_REF_FUT_VOLUME_VPOC" else frames[date]["oi"][frames[date]["oi"].family==family];output+=transitions(q,family,date,futures,fe if family.startswith(("NIFTY_","FUT_")) else oe,config["bin_points"])
