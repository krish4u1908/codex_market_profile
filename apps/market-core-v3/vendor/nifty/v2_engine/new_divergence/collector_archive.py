...
Only NIFTY index, its metadata-selected futures contract, and compact OI
...
def _is_index_symbol(symbol: str) -> bool:
    ...
    return leaf in {"NIFTY50-INDEX", "NIFTY-INDEX"}


def _is_nifty_future(symbol: str) -> bool:
    leaf = symbol.upper().rsplit(":", 1)[-1]
    return (
        leaf.startswith("NIFTY")
        and not leaf.startswith("NIFTYBANK")
        and leaf.endswith("FUT")
        and not leaf.endswith(("CEFUT", "PEFUT"))
    )
...
class CollectorArchiveAdapter:
    ...
    def _consume_startup(self, handle: BinaryIO) -> None:
        ...
        futures = [
            ...
            if _is_nifty_future(str(symbol))
        ]
        indexes = [
    ...
    def _raw_event(self, row: Mapping[str, object], member: str, line_number: int, raw: bytes) -> MarketEvent | None:
        ...
        elif _is_nifty_future(symbol):
            selected = self.selected_futures_symbol
            if selected is None:
                raise ActiveContractUnavailable(
                    "active NIFTY Futures is absent from startup metadata; provide futures_symbol"
                )
            if symbol != selected:
                return None
    ...
    def _future_oi_event(
        ...
        candidates = [
            ...
            if isinstance(values, Mapping) and _is_nifty_future(str(symbol))
        ]
        if requested and _is_nifty_future(requested):
    ...
    def _option_pressure_event(
        ...
        if selected_expiry is None:
            raise ValueError("selected NIFTY option chain has no expiry identity")
        ...
        for option in chain:
            ...
            if not leaf.startswith("NIFTY") or leaf.startswith("NIFTYBANK"):
                continue
            option_type = str(option.get("option_type", "")).upper()
            if option_type not in {"CE", "PE"}:
                continue
            if not _finite(option.get("oi")) or not _finite(option.get("oich")):
                raise ValueError("selected NIFTY option row has incomplete OI evidence")
            oi = float(option["oi"])
            change = float(option["oich"])
            if not symbol or symbol in seen_symbols:
                raise ValueError("selected NIFTY option chain has duplicate/missing symbols")
            if not _finite(option.get("strike_price")):
                raise ValueError("selected NIFTY option row has no finite strike")
            price = option.get("ltp")
            if price is not None and not _finite(price):
                raise ValueError("selected NIFTY option row has a non-finite price")
            volume = option.get("volume")
            if volume is not None and not _finite(volume):
                raise ValueError("selected NIFTY option row has a non-finite volume")
        ...
        return MarketEvent(
            ...
            session=self.session,
            kind=kind,
            symbol=str(row.get("input_symbol", "NIFTY")),
    ...
    def stream(self) -> Iterator[MarketEvent]:
        with tempfile.TemporaryDirectory(prefix="nifty-normalized-") as temporary:
