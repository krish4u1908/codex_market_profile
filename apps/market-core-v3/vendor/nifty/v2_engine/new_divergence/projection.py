...
from .directional_prediction import prediction_series, SERIES_FIELDS
...
def nifty_0945_close_reference(
    observations: Iterable[Mapping[str, object]],
    *,
    session: str,
    ...
    """Select the last fresh synchronized NIFTY Index receipt at/before 09:45 IST."""
    ...
    if not unique:
        return {**base, "status": "MISSING_0945_NIFTY_CLOSE"}
    ...
    return {
        **base,
        "status": "VALID_0945_NIFTY_CLOSE" if valid else "STALE_0945_NIFTY_CLOSE",
        "age_ms": age_ms,
...
def close_0945_strike_selection(
    ...
    """Select and freeze ATM plus three OTM contracts from the 09:45 NIFTY close."""

    unavailable = {
        "available": False,
        "rule": "NIFTY_0945_CLOSE_ATM_PLUS_THREE_OTM_FIXED",
        "reason": "MISSING_OR_STALE_0945_NIFTY_CLOSE",
        "volume_retained": False,
        "CE": [],
        "PE": [],
    }
    if close_reference.get("status") != "VALID_0945_NIFTY_CLOSE":
        return {
            **unavailable,
            "reason": str(close_reference.get("status") or "MISSING_OR_STALE_0945_NIFTY_CLOSE"),
            "reference_close": dict(close_reference),
        }
    try:
        reference_price = float(close_reference.get("price"))
        selection_start = parse_instant(
            close_reference.get("target"), field="09:45 NIFTY-close target"
        )
    except (TypeError, ValueError) as error:
        raise ValueError("09:45 NIFTY-close reference is invalid") from error
    if not math.isfinite(reference_price):
        raise ValueError("09:45 NIFTY-close reference has a non-finite price")
...
def _inventory_unavailable(
    ...
    *,
    ...
    return {
        ...
        "coordinate": "CAUSAL_NIFTY_INDEX_REFERENCE_PRICE_BIN",
...
def session_payload(
    ...
    *,
    enable_oi_vpoc: bool = True,
    enable_volume_profile: bool = True,
    direction_strategy: str = 'corrected',
) -> dict[str, object]:
    ...
    close_reference = nifty_0945_close_reference(
        observations,
    ...
    direction_rows = prediction_series(
        price_rows,
        oi_rows,
        cash_rows,
        intraday_rows,
        strategy=direction_strategy,
    )
    ...
    return {
        ...
        "directional_prediction": {
            **_pack(
                direction_rows,
                SERIES_FIELDS + ('horizon_minutes', 'availability_clock', 'causal_as_of', 'core_score'),
            ),
            "schema": "NIFTY_AGGRESSIVE_DIRECTION_SERIES_V2",
            "strategy": direction_strategy,
            "classification": "EXPERIMENTAL_FORCED_DIRECTION_NOT_VALIDATED",
            "horizon_minutes": 5,
            "primary_inputs": [
                "INDEX_PRICE", "FUTURES_PRICE", "BASIS", "FUTURES_OI",
                "WEIGHTED_CASH_SLOPE", "INDIA_VIX_SLOPE",
            ],
            "vpoc_role": "CONFIDENCE_MODIFIER_ONLY",
            "forced_direction": False,
            "direction_policy": "UP_DOWN_WHEN_CONTIGUOUS_PRICE_HISTORY_IS_AVAILABLE",
            "replay_pause": False,
        },
        "states": _pack(({
            "t": row["as_of"],
        ...
        "rendering_policy": {
            ...
            "option_flow_strikes": "NIFTY_0945_CLOSE_ATM_PLUS_THREE_OTM_FIXED",
            ...
            "cash_vix_start": "09:45:00_ASIA_KOLKATA_HARD_BOUNDARY_WITH_0915_ROLLING_WARMUP",
            "weighted_cash": "NORMALIZED_CONSTITUENT_WEIGHTED_RETURN_FROM_FIRST_SESSION_OPEN",
            "india_vix": "INSTRUMENT_CLASS_VIX_OR_SYMBOL_CONTAINS_INDIAVIX",
            "cash_vix_rolling_minutes": 5,
            "cash_vix_engine_input": False,
            "direction_engine": "SERVER_SIDE_CAUSAL_ONE_DECISION_PER_COMPLETED_MINUTE",
            "direction_engine_vpoc_role": "CONFIDENCE_MODIFIER_ONLY",
            "direction_engine_replay_pause": False,
            "inventory_context_engine_input": False,
            "inventory_context_coordinate": "CAUSAL_NIFTY_INDEX_REFERENCE_PRICE_BIN",
