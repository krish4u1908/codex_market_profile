...
from v2_engine.inventory.engine import (
    FAMILIES,
    oi_events,
    price_events,
)
from v2_engine.raw_io.reader import load_oi, select_contracts
from v2_engine.runtime.timestamps import parse_timestamp_series
...
VOLUME_PROFILE_FAMILY = "NIFTY_REF_FUT_VOLUME_VPOC"
                ...
                if row["kind"] == "raw" and isinstance(record, dict):
                    ...
                    if isinstance(message, dict) and isinstance(message.get("symbol"), str):
                        ...
                        if (
                            ...
                            and (leaf.endswith("-INDEX") or leaf.endswith("FUT"))
                        ):
...
def _select_index_symbol(symbols: Counter[str]) -> str:
    preferred = "NSE:NIFTY50-INDEX"
    if symbols[preferred]:
        return preferred
    candidates = [
        symbol
        for symbol in symbols
        if symbol.upper().rsplit(":", 1)[-1] in {"NIFTY50-INDEX", "NIFTY-INDEX"}
    ]
    if not candidates:
        return ""
...
def _analyze_session(
    ...
    if not index_symbol:
        reasons.append("MISSING_NIFTY_INDEX")
    try:
    ...
    for family in FAMILIES:
        frame = price_frame if family == "NIFTY_REF_FUT_VOLUME_VPOC" else oi_frame[oi_frame.family.eq(family)]
    ...
    contracts = [
        {
            ...
            "symbols": (
                ...
                and family_profiles[row.revision_id].contracts
                    ...
                    [row.futures_symbol]
                    if family.startswith(("NIFTY_", "FUT_")) and row.futures_symbol
    ...
    expiries = [
        {
            ...
            "expiries": (
                ...
                and family_profiles[row.revision_id].expiries
                    ...
                    for value in (
                        row.futures_expiry
                        if family.startswith(("NIFTY_", "FUT_"))
