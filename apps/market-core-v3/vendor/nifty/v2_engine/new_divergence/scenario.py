"""Deterministic, causal NIFTY inventory scenario classification."""
...
def inventory_scenario(bundle: Mapping[str, object]) -> dict[str, object]:
    ...
    futures_climaxes = [
        dict(row) for row in volume_source
        if isinstance(row, Mapping) and is_volume_climax(row)
    ] if isinstance(volume_source, list) else []
    ...
    metrics = {
        ...
        "futures_volume_climax_ratio": (
            futures_climaxes[-1].get("ratio") if futures_climaxes else None
        ),
    }
    ...
    if 0 < peak_index < len(oi15) - 1:
        ...
        if (
            futures_climaxes
            and build_oi > 0 and unwind_oi < 0
            ...
            return _result(
                ...
                [
                    ...
                    f"NIFTY failed back below CE control {ce_control:g}.",
                    f"Basis changed {basis_move:+g} over five minutes, showing Futures relative weakness.",
                    f"NIFTY Futures volume reached {float(futures_climaxes[-1]['ratio']):.6g}x its eligible baseline.",
                ],
                ...
                metrics, ["NIFTY_FUTURES_VOLUME_CLIMAX", "OBSERVED_LONG_BUILDUP", "FUTURES_LONG_LIQUIDATION", "FAILED_CE_CONTROL", "BASIS_CONTRACTION"],
    ...
    if long_setup:
        ...
        return _result(
            ...
            [
                f"NIFTY changed {index_move:+g} and Futures changed {futures_move:+g} over five minutes.",
    ...
    return _result(
        ...
        [
            f"NIFTY changed {index_move:+g}, Futures {futures_move:+g}, Futures OI {oi_move:+g}, and basis {basis_move:+g} over five minutes."
        ],
