...
class LiveCashVixSource:
    def __init__(self,data_root,session,journal):
        ...
        self.rows=[]; self.seen=set(); self.signature=None; self.samples=[]; self.status='AWAITING_MINUTE_FEED'
    ...
    def poll(self,as_of):
        ...
        try:
            ...
            added=[]
            # Recheck publication time on every poll, even when the CSV bytes
            # have not changed since a poll just before the 8-second cutoff.
