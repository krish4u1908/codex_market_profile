...
from .clock import IST, iso_utc, parse_instant
...
class LiveAuthority:
    ...
    def __init__(self, state_root: Path, session: date, config: EngineConfig | None = None):
        ...
        self.cash_vix_source = None
        self._watermark = None
        self._profile_minute = None
        self.direction_path = self.root / 'direction_publications_v1062.jsonl'
        self.direction_history = ([json.loads(line) for line in self.direction_path.read_text().splitlines()]
                                  if self.direction_path.exists() else [])
    ...
    def publish_due_direction(self) -> None:
        """Publish on the runtime clock even when no browser is connected."""
        with self._condition:
            if self._watermark is None:
                return
            cutoff = (self._watermark-timedelta(seconds=8)).replace(second=0,microsecond=0)
            local = cutoff.astimezone(IST)
            if local.date() != self.session or not (9*60+46 <= local.hour*60+local.minute <= 15*60+30):
                return
            if self.direction_history and parse_instant(self.direction_history[-1]['input_cutoff']) >= cutoff:
                return
            # Missed live minutes are not backfilled with a historical timestamp.
            if not 0 <= (datetime.now().astimezone()-cutoff).total_seconds() <= 30:
                return
            self._profile_locked()

    def _profile_locked(self) -> dict[str, object]:
        observations = [row.to_dict() for row in self.engine.observations]
        evidence = [row.to_dict() for row in self.engine.evidence_snapshots]
        now = datetime.now().astimezone()
        through = self._watermark or (self.events[-1].receipt_timestamp if self.events else now)
        minute = (through-timedelta(seconds=8)).replace(second=0,microsecond=0)
        if self._profile_cache is None or self._profile_cache_sequence != self._sequence or self._profile_minute != minute:
            cash_rows = self.cash_vix_source.poll(now) if self.cash_vix_source else []
            self._profile_cache = live_profile_projection(
                self.events, observations, evidence,
                session=self.session, config=self.config,
                cash_vix_rows=cash_rows,through=through,direction_history=self.direction_history,
                actual_publication=now,
            )
            decision=self._profile_cache.get('directional_prediction')
            if decision and (not self.direction_history or decision['decision_id']!=self.direction_history[-1]['decision_id']):
                decision['input_availability_cutoff']=decision['published_at']
                decision['published_at']=decision['t']=iso_utc(datetime.now().astimezone())
                decision['availability_clock']='ACTUAL_LIVE_CALCULATION_COMPLETION'
                with self.direction_path.open('a',encoding='utf-8') as handle:
                    handle.write(json.dumps(decision,sort_keys=True)+'\n'); handle.flush(); os.fsync(handle.fileno())
                self.direction_history.append(decision)
            self._profile_cache['cash_vix_source_status']=self.cash_vix_source.status if self.cash_vix_source else 'NOT_CONFIGURED'
            self._profile_cache_sequence = self._sequence
            self._profile_minute = minute
        return dict(self._profile_cache)

    @staticmethod
    def _public_profile(profile: Mapping[str, object]) -> dict[str, object]:
        """Return the browser-safe live market-profile projection."""
        return dict(profile)
    ...
    def commentary_snapshot(self) -> dict[str, object]:
        ...
        with self._condition:
            return {
                ...
                "profile": self._public_profile(self._profile_locked()),
            }
