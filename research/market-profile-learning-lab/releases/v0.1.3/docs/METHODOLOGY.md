...
V0.1.3 labels also retain the ordered future Index receipts inside the sealed
label boundary. Candidate workspaces never receive these paths. They are used
only to determine whether a published level was touched and then reacted by
the configured amount before it breached by the configured amount. A touch
without either resolution is reported separately; order cannot be inferred
from future high/low extrema alone.
...
coverage records how often an agent avoids `ROTATION`. Confusion matrices,
actual class support, calibration bins, and neutral prediction rates are
reported for every candidate and horizon.

Every score is also computed per session. V0.1.3 reports session win counts,
leave-one-session-out stability, and a deterministic session-block bootstrap
interval. Episodes are never bootstrapped as if they were independent dates.

The legacy level-extreme metric records whether one of the ranked
support/resistance levels lies within the configured tolerance of the
subsequent path low/high. The ordered reaction metric separately classifies
`REACTION_BEFORE_BREACH`, `BREACH_BEFORE_REACTION`, `TOUCHED_UNRESOLVED`,
`AMBIGUOUS_SAME_RECEIPT`, and `UNTESTED`.
...
The legacy aggregate gate remains visible for comparison. V0.1.3 selection is
horizon-specific and uses a predeclared allowlist. A specialist must satisfy
the configured balanced-accuracy margin, composite margin, Brier boundary,
session-win fraction, leave-one-session-out fraction, directional coverage,
actual-class support, reaction-test count, and reaction-rate margin against the
strongest same-horizon deterministic baseline.

## Promotion boundary

The internal holdout can be opened once only when at least one predeclared
horizon specialist passes every V0.1.3 validation gate. The open marker binds
the complete candidate inventory, validation-selection hash, cases hash,
labels hash, and selected candidate/horizon list. Holdout evaluation is limited
to those specialists and the same-horizon baselines.
