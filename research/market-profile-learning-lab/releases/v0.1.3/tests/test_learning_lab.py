...
from banknifty_market_profile_lab.scoring import (
    _level_test,
    candidate_inventory,
    evaluate_split,
    import_frozen_candidates,
)
...
class LearningLabEndToEndTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        atomic_json(self.config, {
            ...
            "evaluation_v013": {
                "horizon_candidate_allowlist": {},
                "minimum_balanced_accuracy_margin": -1.0,
                "minimum_composite_margin": -1.0,
                "maximum_brier_worsening": 1.0,
                "minimum_session_win_fraction": 0.0,
                "minimum_loo_pass_fraction": 0.0,
                "minimum_directional_coverage": 0.0,
                "minimum_present_actual_classes": 1,
                "minimum_level_reaction_tests": 0,
                "minimum_level_reaction_rate_margin": -1.0,
                "session_bootstrap_replicates": 20,
                "session_bootstrap_seed": 13,
            },
        })
    ...
    def test_end_to_end_and_holdout_freeze(self) -> None:
        ...
        selection = json.loads(
            (learning_run / "scores/validation_selection.json").read_text(encoding="utf-8")
        )
        self.assertEqual(
            selection["schema"], "BANKNIFTY_MARKET_PROFILE_VALIDATION_SELECTION_V2"
        )
        self.assertTrue(selection["eligible_specialists"])
        score = validation["scores"][0]
        self.assertIn("session_diagnostics", score)
        self.assertIn("level_reaction_rate", score)
    ...
    def test_ordered_level_reaction_distinguishes_reaction_and_breach(self) -> None:
        reaction = _level_test(
            [100.0],
            {"ordered_future_path": [
                {"t": "2026-01-01T00:00:01Z", "p": 110.0},
                {"t": "2026-01-01T00:00:02Z", "p": 100.0},
                {"t": "2026-01-01T00:00:03Z", "p": 126.0},
                {"t": "2026-01-01T00:00:04Z", "p": 80.0},
            ]},
            side="support",
            tolerance=15.0,
            reaction_points=25.0,
            breach_points=15.0,
        )
        self.assertEqual(reaction["status"], "REACTION_BEFORE_BREACH")
        breach = _level_test(
            [100.0],
            {"ordered_future_path": [
                {"t": "2026-01-01T00:00:01Z", "p": 100.0},
                {"t": "2026-01-01T00:00:02Z", "p": 84.0},
                {"t": "2026-01-01T00:00:03Z", "p": 130.0},
            ]},
            side="support",
            tolerance=15.0,
            reaction_points=25.0,
        ...
        self.assertEqual(breach["status"], "BREACH_BEFORE_REACTION")

    def test_frozen_candidate_import_preserves_hashes(self) -> None:
        source_run = self.root / "source-learning-run"
        target_run = self.root / "target-learning-run"
        for destination in (source_run, target_run):
            build_dataset(
                run_root=self.run_root,
                gui_root=self.gui_root,
                config_path=self.config,
        ...
        source_candidates = create_seed_candidates(source_run)
        selected_id = source_candidates[0]["candidate_id"]
        imported = import_frozen_candidates(
            target_run,
            source_run=source_run,
            candidate_ids=[selected_id],
        )
        self.assertEqual(imported[0]["candidate_id"], selected_id)
        target_inventory = candidate_inventory(target_run)
        self.assertEqual(len(target_inventory), 1)
        self.assertEqual(
            target_inventory[0]["agent_spec_sha256"], source_candidates[0]["sha256"]
        )
