...
def validate_config(config: Mapping[str, object]) -> dict[str, object]:
    ...
    for key in (
        ...
        "level_reaction_points",
        "level_breach_points",
    ):
        if finite(config.get(key)) is None:
            raise ValueError(f"configuration field {key} is not finite")
    gate = config.get("evaluation_v013")
    if not isinstance(gate, Mapping):
        raise ValueError("evaluation_v013 configuration is missing")
    numeric_gate_fields = (
        "minimum_balanced_accuracy_margin",
        "minimum_composite_margin",
        "maximum_brier_worsening",
        "minimum_session_win_fraction",
        "minimum_loo_pass_fraction",
        "minimum_directional_coverage",
        "minimum_present_actual_classes",
        "minimum_level_reaction_tests",
        "minimum_level_reaction_rate_margin",
        "session_bootstrap_replicates",
        "session_bootstrap_seed",
    )
    for key in numeric_gate_fields:
        if finite(gate.get(key)) is None:
            raise ValueError(f"evaluation_v013 field {key} is not finite")
    allowlist = gate.get("horizon_candidate_allowlist", {})
    if not isinstance(allowlist, Mapping):
        raise ValueError("horizon_candidate_allowlist must be an object")
    for horizon, values in allowlist.items():
        if str(horizon) not in {"5", "15", "30"} or not isinstance(values, list):
            raise ValueError("horizon_candidate_allowlist is invalid")
...
def _future_label(
    ...
    return {
        ...
        "ordered_future_path": [
            ...
            for row, value in zip(path, path_prices)
    ...
    for split, dates in config["splits"].items():
        for session in dates:
            ...
            for group in groups:
                ...
                label = {
                    "schema": "BANKNIFTY_MARKET_PROFILE_SEALED_LABEL_V2",
                    "case_id": case_id,
