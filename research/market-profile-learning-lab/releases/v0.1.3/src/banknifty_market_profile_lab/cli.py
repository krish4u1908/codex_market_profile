...
from .scoring import evaluate_split, import_frozen_candidates
...
def parser() -> argparse.ArgumentParser:
    ...
    imported = commands.add_parser(
        "import-candidates",
        help="copy exact frozen candidates from an earlier sealed learning run",
    )
    imported.add_argument("--learning-run", type=Path, required=True)
    imported.add_argument("--source-run", type=Path, required=True)
    imported.add_argument("--candidate-id", action="append", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "import-candidates":
            _json({"candidates": import_frozen_candidates(
                args.learning_run.resolve(),
