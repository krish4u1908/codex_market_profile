...
def generate_report(run_root: Path) -> Path:
    ...
    lines.extend([
        ...
        "## Metric boundary",
        "",
        "The level metric asks whether a predicted level lies near the future path extreme. It does not prove that the level caused support or resistance, nor that a trade at that level would have been executable or profitable.",
    ])
    ...
    atomic_json(run_root / "reports" / "report_manifest.json", {
        "schema": "BANKNIFTY_MARKET_PROFILE_REPORT_MANIFEST_V1",
